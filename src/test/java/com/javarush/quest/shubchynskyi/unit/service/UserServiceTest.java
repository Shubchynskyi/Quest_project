...
class UserServiceTest {
    ...
    void should_ReturnTrue_When_LoginExists() {
        when(userRepository.exists(ArgumentMatchers.any())).thenReturn(true);

        boolean exists = userService.isLoginExist(TEST_USER_LOGIN);

        assertTrue(exists);
        verify(userRepository, times(1)).exists(ArgumentMatchers.any());
    }

    @Test
    void should_ReturnFalse_When_LoginDoesNotExist() {
        when(userRepository.exists(ArgumentMatchers.any())).thenReturn(false);
